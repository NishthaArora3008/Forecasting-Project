...
image <- image_read("https://thumbs.dreamstime.com/b/businessman-looking-far-economic-forecasting-concept-businessman-looking-far-economic-forecasting-concept-153410971.jpg")
image
...
Figure \@ref(fig:fig22) below shoes the residual values for both models, when compared to original ABS data.

```{r fig22, fig.cap="Difference: ABS orignal vales - mean of forecasted values"}
both_best2 |> 
  forecast(h=51) |> 
...
In figure \@ref(fig:fig23), ACF for ETS seems to have almost 0 value lags after it decays at 12.

```{r fig23, fig.cap="ACF of residuals"}
both_best2 |> 
  forecast(h=51) |> 
  mutate(Turnover.ABS= rep(abs$Turnover,times=2)) |> 
  mutate(.resid=Turnover.ABS-.mean) |> 
...
Figure \@ref(fig:fig24) shows that both the models form a somewhat bi nodal histograms but it it more clear in the case of arima.

```{r fig24, fig.cap="Histogram of residuals"}
both_best2 |> 
  forecast(h=51) |> 
  mutate(Turnover.ABS= rep(abs$Turnover,times=2)) |> 
  mutate(.resid=Turnover.ABS-.mean) |> 
