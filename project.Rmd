...
Observing figure \@ref(fig:fig9), we can say that error and seasonality are multiplicative. There is somewhat an upward trend which can be called as additive. The appropriate models could be (MAM), (MNM) or (MAdM). Also here the seasonality component we can also use the additive compoment to the sesonality which leads to ETS models (M,N,A) or (A,Ad,A)
...
We observe that the models MAdM and ANA have the same results other than just one where the valuse for ANA is the lowest and hence we select ETS (ANA) as the best model.
...
The best ETS model chosen is ANA
...
And the best ARIMA model is (102)(011)
...
Here the p value is <0.05, suggesting that the values are autocorrelated, rejecting the null hypothesis.
...
- Both the models show somehwat a normal distribution and white noise in the data (figure \@ref(fig:fig12) and \@ref(fig:fig15).
...
Comparing figure \@ref(fig:fig18) with \@ref(fig:fig13) which is the overall forecast and comparison between both best bodels too, it shows that prediction intervals are narrow in best ARIMA.
...
The accuracy values are lower for best arima model, suggessting better forecasts.
...
\@ref(fig:fig19) shows forecast of two best models on full dataset with 80% prediction interval.
...
- ETS was unpredictible as it predicted ANA and could not handle trend well.
